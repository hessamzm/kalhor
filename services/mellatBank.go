...
var melatinfo models.MellaForm
...
func (c *Client) RefundToPANRequest(orderId, saleOrderId, saleReferenceId, refundAmount int64, refundReason, panNumber string) (string, error) {
	soap, err := gosoap.SoapClient(serviceURL)
	if err != nil {
		return "", err
	}
	...
	err = soap.Call("bpRefundToPANRequest", params)
	if err != nil {
		return "", err
	}
...
func (m *MellatService) InsertMellatFormGorm(refId, phoneNumber, body, encPan, enc string) error {

	form := models.MellaForm{
		RefID:       refId,
	...
	if err := facades.Orm().Query().Model(&melatinfo).Save(&form); err != nil {
		log.Println("Failed to insert Mellat form data:", err)
		return err
	}
	return nil
}

func (c *Client) MakeForm(refId, phoneNumber, body, encPanuncode, encmellinumber string, mellatService *MellatService) string {
	encPan := utils.MellatEncrypt(encPanuncode)
	enc := utils.MellatEncrypt(encmellinumber)

	// You can use `mellatService` to insert data into the database here
	err := mellatService.InsertMellatForm(refId, phoneNumber, body, encPan, enc)
	if err != nil {
		// Handle error if the insert fails
		fmt.Println("Error inserting form data:", err)
	}

	return fmt.Sprintf(
		`<form id="payForm" name="payForm" action="%s" method="post" target="_self">
					<input type="text" id="RefId" name="RefId" value='%s' />
					<input type="hidden" id="HiddenMode" name="HiddenMode" value='1' />
					<input type="hidden" id="encPan" name="EncPan" value='%s' />
					<input type="hidden" id="enc" name="EncPan" value='%s' />
					<input type="hidden" id="mobileNo" name="EncPan" value='%s' />
					<div>%s</div>
